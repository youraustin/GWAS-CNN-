...
myroot1 = strcat('E:\final_results\space\', num2str(Sizek), '\', DataSetName1, '\', 'RowPrime\', 'control');
if ~isdir(myroot1) 
    mkdir(myroot1);
end

myroot2 = strcat('E:\final_results\space\', num2str(Sizek), '\', DataSetName1, '\', 'RowPrime\', 'uncontrol');
if ~isdir(myroot2) 
    mkdir(myroot2);
end
%% [row, column]=size(X)
